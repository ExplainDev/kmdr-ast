...
import { Decorators, Highlight } from "./interfaces";
import DecoratorReact from "./decoratorReact";

class HighlightReact extends DecoratorReact implements Highlight {
  constructor(decorators: Decorators) {
    super(decorators);
  }

  public highlight(query: string, ast: NodeAST): any {
    let decoratedNodes = [];
    let currentToken = 0;
    let wordInRange = "";
    let inRange = false;
    ...
    for (let pos = 0; pos < query.length; pos++) {
      ...
      if (
        ...
        if (
          ...
          decoratedNodes.push(super.decorate(tokens[currentToken]));
          inRange = false;
          wordInRange = "";
          currentToken += 1;
        }
